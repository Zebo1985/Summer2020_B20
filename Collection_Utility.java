...
public class Collection_Utility {
    public static void main(String[] args) {
      ...
      students.addAll(Arrays.asList("Aslan","Cristina","Zelina","Iman","Fatima","Mohammed"));
                                     //   0       1          2       3       4        5
        ...
        items.addAll(Arrays.asList("Coffe","Eggs","Coffe", "Battery","Battery","Battery"));
        int count = Collections.frequency(items,"Battery");
        System.out.println(count);
        ArrayList<String>unique = new ArrayList<>();
        for(String each: items) {
            int n = Collections.frequency(items, each);
            if (n == 1) {
                unique.add(each);
            }
        }
